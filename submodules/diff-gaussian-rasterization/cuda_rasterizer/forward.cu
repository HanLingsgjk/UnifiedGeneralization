...
renderCUDA(
	...
	float* __restrict__ out_depthn,
	float* __restrict__ out_depthf)
{
    ...
    float Dn = 1.0f;  // Median Depth. TODO: This is a hack setting max_depth to 15
    float Df = 30.0f;  // Median Depth. TODO: This is a hack setting max_depth to 15
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
            ...
            if (T > 0.1f && test_T < 0.1)
			{
			    float dep = collected_depth[j];
				Dn = dep;
			}
            if (T > 0.9f && test_T < 0.9)
			{
			    float dep = collected_depth[j];
				Df = dep;
			}
	...
	if (inside)
	{
		...
		out_depth[pix_id] = D;
		out_depthn[pix_id] = Dn;
		out_depthf[pix_id] = Df;
	}
}
