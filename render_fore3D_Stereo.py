...
def get_sterero_pose(c2w,bl):
    ...
    xgo = baseline+ (torch.rand(1)-0.5)*0.2
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, scale_factor):
    ...
    dataroot = '/home/lh/all_datasets/MIPGS10K_stereo_foremintest '
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        for viz in range(Nums):
            ...
            c2wf1, c2wf2 = get_sterero_pose(c2w,bl=viz+1)  # 生成新的光流视角
            # TODO 渲染第一帧光流
            ...
            for mask2 in masklist2:
                ...
                if flagconf and vmax > 0.90 and numl < 0.3 and onum < 0.3 and ~(
                    ...
                    if n22 > 0.95:
                        ...
                        '''
                        plt.imshow(RGB*mask2[:,:,None])
                        plt.show()
                        plt.imshow(RGBF1*masklist[dimmax][:,:,None])
                        plt.show()
