...
def viewtrans(view,c2w):
    ...
    zone[0,3]=1
    c2w=np.concatenate([c2w,zone],axis=0)
    view.world_view_transform = torch.from_numpy(c2w).cuda().inverse().transpose(0, 1)
    ...
    return view
def get_sterero_pose(c2w,bl):
    #构建双目立体匹配用的视差对  bl 基线倍率
    baseline = 0.5*bl
    ...
    xgo = baseline+ (torch.rand(1)-0.5)*0.25


    c2wn2[:, :3] = torch.matmul(Rxyz, c2w[:, :3].detach().cpu())
    c2wn2[:, 3:] = c2w[:, 3:].detach().cpu() - xgo * directx
    c2wn2 = c2wn2.numpy()
    c2wn3[:, :3] = torch.matmul(Rxyz, c2w[:, :3].detach().cpu())
    c2wn3[:, 3:] = c2w[:, 3:].detach().cpu() + xgo * directx
    c2wn3 = c2wn3.numpy()

    c2wl = c2wn3
    c2wr = c2wn2
    c2wm = c2w.detach().cpu().numpy()
    return c2wl,c2wr#这个标记写反了
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, scale_factor):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        for viz in range(Nums):
            ...
            #depth1 = renders["depthmean"].detach().cpu().numpy()
            ...
            fileid = str(kit).zfill(5)+'_'+str(1+viz).zfill(2)+'.png'
            if ifsave:
            ...
            #depth2 = renderF1["depthmean"].detach().cpu().numpy()
            ...
            occused = occall < 0.016

            ddcmean = ddcs[occused>0].mean()
            ddc_list.append(ddcmean)
            ...
            if ifshow:
                ...
                plt.imshow(pah)
                plt.show()
                plt.imshow(vis.flow2rgb(flows.clip(-400,0)))
                plt.show()
                plt.imshow(flows[:,:,0], cmap='turbo')
                plt.show()
                plt.imshow(ddcs.clip(0,0.05))
                plt.show()
                ddcs[occused==0] = 0
                plt.imshow(occused,cmap='gray')
                plt.show()
                plt.imshow(S>0.7)
                plt.show()
            ...
            print('FlowNum:', kit, 'ddc:', ddcmean, 'Disf', abs(flowf1.permute(1,2,0).detach().cpu().numpy()).mean())
    ddc_list = np.array(ddc_list)
    ddcmeanu = np.mean(ddc_list[ddc_list<100])
    print('DDCmean:', ddcmeanu )
