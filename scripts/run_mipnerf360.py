...
def train_scene(gpu, scene, factor):
    cmd1 = f"cd .."
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s /media/lh/extradata/360_v2/{scene} -m {output_dir}/{scene} --eval -r {factor} --port {6009+int(gpu)} --kernel_size 0.1"
    print(cmd)
    if not dry_run:
        os.system(cmd1)
        os.system(cmd)
