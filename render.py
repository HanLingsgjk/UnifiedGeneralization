...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, kernel_size, scale_factor):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        renderall = render(view, gaussians, pipeline, background, kernel_size=kernel_size)
        rendering = renderall["render"]
        depth = renderall["depthmid"].detach().cpu().numpy()
        depths_vis = getvis(depth[0])

        depths = torch.from_numpy(depths_vis).permute(2,0,1)

        gt = view.original_image[0:3, :, :]
        #torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
        torchvision.utils.save_image(depths, os.path.join(render_path, '{0:05d}'.format(idx) + ".png"))
        torchvision.utils.save_image(rendering, os.path.join(gts_path, '{0:05d}'.format(idx) + ".png"))
