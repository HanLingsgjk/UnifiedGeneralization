...
def img_warping(ref_pose, src_pose, virtual_pose_ref_depth, virtual_intrinsic, src_img):
    ...
    pts_in_3D_tgt = rel_pose[:3, :3] @ pts_in_3D.view(-1, 3).permute(1, 0) + rel_pose[:3, 3].unsqueeze(-1).repeat(1,
                                                                                                                  ht * wd)
    ...
    warped_ref_img = torch.nn.functional.grid_sample(src_img.unsqueeze(0), pts_in_tgt.unsqueeze(0), mode='nearest',
                                                     padding_mode="zeros")
...
def reproject_with_depth(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,RGB1,RGB2):
    batch, height, width = depth_ref.shape
    # ref深度投影到相机坐标系3Dref->从ref投影到src 3Dsrc'->投影到平面采样src深度 ->转为3Dsrcd'->从src投影到ref
    ...
    #转到相机坐标系
    # source 3D space  s转到r的点
    xyz_src = torch.matmul(torch.matmul(torch.inverse(extrinsics_src), extrinsics_ref),
                           torch.cat((xyz_ref, torch.ones_like(x_ref).to(x_ref.device).unsqueeze(1)), dim=1))[:, :3]#转到参考帧坐标系
    # source view x, y

    K_xyz_src = torch.matmul(intrinsics_src, xyz_src)#
    xy_src = K_xyz_src[:, :2] / K_xyz_src[:, 2:3]#参考帧在源帧的投影

    ## step2. reproject the source view points with source view depth estimation
    # find the depth estimation of the source view
    x_src = xy_src[:, 0].reshape([batch, height, width]).float()
    y_src = xy_src[:, 1].reshape([batch, height, width]).float()
    z_src = K_xyz_src[:, 2:3].reshape([batch, height, width]).float()
    # print(x_src, y_src)  用这些投影点的坐标采样深度？
    RGB1u = torch.from_numpy(RGB1).permute(2,0,1).cuda()
    RGB2u = torch.from_numpy(RGB2).permute(2,0,1).cuda()
    sampled_depth_src = bilinear_sampler(depth_src.view(batch, 1, height, width),
                                         torch.stack((x_src, y_src), dim=-1).view(batch, height, width, 2))
    sampled_rgb_src = bilinear_sampler(RGB2u.view(batch, 3, height, width),
                                         torch.stack((x_src, y_src), dim=-1).view(batch, height, width, 2))
    depthout = z_src
    # source 3D space  再将这些点基于src深度反投影到src相机坐标系
    # NOTE that we should use sampled source-view depth_here to project back
    xyz_src = torch.matmul(torch.inverse(intrinsics_src),
                           torch.cat((xy_src, torch.ones_like(x_ref).to(x_ref.device).unsqueeze(1)),
                                     dim=1) * sampled_depth_src.reshape(batch, 1, -1))

    # reference 3D space  再将这些3D点反投影回ref
    xyz_reprojected = torch.matmul(torch.matmul(torch.inverse(extrinsics_ref), extrinsics_src),
                                   torch.cat((xyz_src, torch.ones_like(x_ref).to(x_ref.device).unsqueeze(1)), dim=1))[:,
                      :3]
    ...
    return depth_reprojected, x_reprojected, y_reprojected, x_src, y_src,depthout,sampled_depth_src[0],sampled_rgb_src


# 参考帧深度，内参，c2w,查找帧深度，内参，c2w,
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref,RGB1, depth_src, intrinsics_src, extrinsics_src,RGB2,
                                thre1=1, thre2=0.01):
    batch, height, width = depth_ref.shape
    y_ref, x_ref = torch.meshgrid(torch.arange(0, height).to(depth_ref.device),
                                  torch.arange(0, width).to(depth_ref.device))
    x_ref = x_ref.unsqueeze(0).repeat(batch, 1, 1)
    y_ref = y_ref.unsqueeze(0).repeat(batch, 1, 1)
    inputs = [depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,RGB1,RGB2]
    outputs = reproject_with_depth(*inputs)
    depth_reprojected, x2d_reprojected, y2d_reprojected, x2d_src, y2d_src,sampled_depth_src,sampled_depth_src2,sampled_rgb_src = outputs
    # check |p_reproj-p_1| < 1  这个是像素误差
    dist = torch.sqrt((x2d_reprojected - x_ref) ** 2 + (y2d_reprojected - y_ref) ** 2)
    # check |d_reproj-d_1| / d_1 < 0.01  这个是深度百分比误差
    ...
    corrd1 = torch.cat([x_ref,y_ref])
    corrd2 = torch.cat([x2d_src, y2d_src])
    flow = corrd2-corrd1
    depth_diff2 = torch.abs(sampled_depth_src - sampled_depth_src2)
    ddcout = depth_diff2/(sampled_depth_src + sampled_depth_src2)
    return mask, depth_reprojected, x2d_src, y2d_src, relative_depth_diff, dist,flow,sampled_rgb_src,sampled_depth_src,corrd2,ddcout
...
def depth_propagation(viewpoint_cam, projected_depth, viewpoint_stack, src_idxs, dataset, patch_size):
    ...
    write_cam_txt(os.path.join(cdata_camera_path, "0.txt"), ref_K.detach().cpu().numpy(),
                  ref_w2c.detach().cpu().numpy(),
    ...
    for idx, src_idx in enumerate(src_idxs):
        ...
        write_cam_txt(os.path.join(cdata_camera_path, str(idx + 1) + ".txt"), src_K.detach().cpu().numpy(),
                      src_w2c.detach().cpu().numpy(),
                      [depth_min, (depth_max - depth_min) / 192.0, 192.0, depth_max])
