...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, kernel_size: float, scaling_modifier = 1.0, override_color = None, subpixel_offset=None):
    ...
    rendered_image, radii,depth,depthn,depthf = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
    ...
    means3D_depth = means3D_depth.repeat(1, 3)  # 渲染深度就是把颜色值换成了点云的中心距离
    render_depth, _,_,_,_ = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = None,
    ...
    '''
    pah = torch.abs(depthf[0] - depthn[0]) / (depthf[0] + depthn[0])
